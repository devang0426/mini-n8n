/**
 * AI Agent Workflow Builder — Phase 6B Verification Master Runner
 * Executes Suite A (GraphQL Integration & Security) and Suite B (Frontend & Unit State) tests.
 */

import { runSuiteA_IntegrationTests, runSuiteB_FrontendUnitTests, TestResult } from './phase6b_workflow_builder.test';
...
async function getOrCreateNhostUserSession(email: string, pass: string) {
  ...
  // 2. Sign in via Nhost Auth API with exponential backoff on rate-limit (429)
  let attempts = 0;
  while (attempts < 10) {
    attempts++;
...
async function main() {
  console.log('==================================================');
  console.log('PHASE 6B — WORKFLOW BUILDER & TRIGGER TEST RUNNER');
  console.log('==================================================\n');

  console.log('--- Setting up Phase 6B Test Fixtures ---');
  ...
  // 1. Obtain Nhost User Sessions with token caching
  ...
  const orgARes = await runAdminSql(`INSERT INTO public.organizations (name, quota_limit, quota_used) VALUES ('Phase 6B Org A', 100, 0) RETURNING id;`);
  const orgA_id = orgARes.body.result[1][0];

  const orgBRes = await runAdminSql(`INSERT INTO public.organizations (name, quota_limit, quota_used) VALUES ('Phase 6B Org B', 100, 0) RETURNING id;`);
  ...
  console.log('\n==================================================');
  console.log('PHASE 6B VERIFICATION RESULTS');
  console.log('==================================================');
...
main().catch((err) => {
  console.error('Fatal error running Phase 6B verification:', err);
  process.exit(1);
});
