/**
 * AI Agent Workflow Builder — approveStep Hasura Action API Route (Phase 5)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const workflow_run_id = body?.input?.workflow_run_id || body?.workflow_run_id;
    const step_run_id = body?.input?.step_run_id || body?.step_run_id;
    const approved = body?.input?.approved !== undefined ? body?.input?.approved : body?.approved;

    const executor = new WorkflowExecutor(runAdminSql);
    const processor = new ActionProcessor(executor, runAdminSql);

    const result = await processor.approveStep({
      workflow_run_id,
      step_run_id,
      approved,
      callerUserId,
    ...
    const errorMsg = sanitizeText(err.message || 'Step approval processing failed.');
    let statusCode = 500;
    let errorCode = err.code || 'ACTION_ERROR';

    if (err instanceof ActionError) {
      statusCode = err.statusCode;
    } else if (err.code === 'WORKFLOW_RUN_NOT_FOUND' || err.code === 'STEP_RUN_NOT_FOUND') {
      statusCode = 404;
    } else if (
      err.code === 'STEP_RUN_MISMATCH' ||
      err.code === 'NOT_APPROVAL_GATE' ||
